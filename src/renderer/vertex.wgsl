...
fn main(
  ...
  // Debug: Just place quads at node positions, fixed size
  let world_pos = node_pos + (quad_pos * node_radius); // Fixed size of 10 units
  output.position = uniforms.viewProj * vec4<f32>(world_pos, 0.0, 1.0);

  output.color = node_color; //vec3<f32>(1.0, 0.0, 0.0); // Force red
  output.uv = quad_uv;
